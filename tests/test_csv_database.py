...
class TestCSVDatabaseExtraCoverage(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_get_table_invalid_csv_raises_storage_error(self) -> None:
    ...
    def test_get_table_invalid_header_raises_storage_error(self) -> None:
        path = Path(self.temp_dir.name) / "broken2.csv"
        path.write_text(
            "wrong_magic,broken2,ID,ID,Name\n",
            encoding="utf-8",
        )
    ...
    def test_get_table_read_oserror_raises_storage_error(self) -> None:
        path = Path(self.temp_dir.name) / "broken3.csv"
        path.write_text(
            "__csvdb__,broken3,ID,ID,Name\n1,\"Test\"\n",
            encoding="utf-8",
        )

        with patch("pathlib.Path.open", side_effect=OSError("read failed")):
            with self.assertRaises(StorageError):
    ...
    def test_save_table_replace_error_raises_storage_error(self) -> None:
        ...
        table = self.db.get_table("replace_fail")
    ...
    def test_build_default_database_does_not_duplicate_existing_patients_table(self) -> None:
        same_db = build_csv_default_database(storage_dir=self.temp_dir.name)
        self.assertIn("patients", same_db.list_tables())

    def test_csv_table_alias_and_roundtrip(self) -> None:
        self.assertTrue(CSVTable is Table)

        self.patients.create_record(self.patient_1)
        data = self.patients.to_dict()
        restored = CSVTable.from_dict("patients", data)
        self.assertEqual(restored.select_records(), [self.patient_1])

    def test_csv_table_from_dict_invalid_inputs(self) -> None:
        with self.assertRaises(ValidationError):
            CSVTable.from_dict("patients", "not a dict")  # type: ignore[arg-type]
    ...
    def test_csv_table_validation_and_sorting_paths(self) -> None:
        table = CSVTable(
            name="docs",
            key_field="id",
            fields=["id", "meta"],
    ...
    def test_csv_table_update_and_delete_error_paths(self) -> None:
        table = CSVTable(
            name="docs2",
            key_field="id",
            fields=["id", "name"],
...
class TestTuiExtraCoverage(unittest.TestCase):
    def test_choose_database_memory_and_csv_branches(self) -> None:
    ...
    def test_tui_main_uses_csv_database_branch(self) -> None:
        fake_db = object()

        with patch("builtins.input", return_value="3"), patch(
            "src.db.tui.build_csv_database",
